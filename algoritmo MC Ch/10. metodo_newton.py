# -*- coding: utf-8 -*-
"""
Calculadora de Sistemas Lineares pelo Método de Gauss-Seidel
===========================================================

Este programa resolve sistemas de equações lineares da forma Ax = b utilizando o
Método de Gauss-Seidel. O método é iterativo e depende de um chute inicial para
convergência.
...
3. Execução do Método de Gauss-Seidel com exibição detalhada de cada iteração.
...
def gauss_seidel(A, b, e, max_i):
    """
    Método iterativo de Gauss-Seidel para resolver um sistema linear Ax = b.
    ...
    # Loop principal que executa as iterações do método de Gauss-Seidel.
    for k in range(1, max_i + 1):
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i != j:
                    soma -= A[i][j] * x_novo[j]  # Subtrai A[i][j] * x_novo[j] da soma.
    ...
    print(f"Solução após {max_i} iterações: {x_novo}\n")
...
def main():
    ...
    3. Executa o Método de Gauss-Seidel para resolver o sistema.
    ...
    # 2x1 + 3x2 + 10x3 = 6
    A = [
        [10, 2, 1],  # Primeira linha da matriz A: 10x1 + 2x2 + 1x3 = 7
        [1, 5, 1],   # Segunda linha da matriz A: 1x1 + 5x2 + 1x3 = -8
        [2, 3, 10]   # Terceira linha da matriz A: 2x1 + 3x2 + 10x3 = 6
    ]
    
    # Vetor de constantes do sistema linear
    b = [7, -8, 6]  # Os valores de b correspondem aos resultados das equações: [7, -8, 6].
    
    print("=== RESOLUÇÃO DE SISTEMAS LINEARES COM GAUSS-SEIDEL ===\n")
    ...
    try:
        solução = gauss_seidel(A, b, e, max_i)  # Chama a função gauss_seidel com os parâmetros fornecidos.
